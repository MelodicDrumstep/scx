...
/// Process task type updates from the ring buffer and update the task_type_by_pid map.
/// This function processes all available entries in the ring buffer in one call.
/// Returns the number of entries processed.
pub fn process_task_type_ring_buffer(skel: &mut BpfSkel) -> Result<usize> {
    const RING_KEY: u32 = 0;
    const TASK_TYPE_RING_SIZE: usize = bpf_intf::consts_TASK_TYPE_RING_SIZE as usize;
    const TASK_TYPE_LC: u8 = bpf_intf::task_type_TASK_TYPE_LC as u8;
    const TASK_TYPE_BE: u8 = bpf_intf::task_type_TASK_TYPE_BE as u8;

    let ring_map = &skel.maps.task_type_ring_buffer;
    let task_type_map = &skel.maps.task_type_by_pid;
    let key = RING_KEY.to_ne_bytes();
    ...
    let mut consumer = u32::from_ne_bytes([
        ring_bytes[8],
        ring_bytes[9],
        ring_bytes[10],
        ring_bytes[11],
    ...
    while consumer != producer {
        let idx = consumer % TASK_TYPE_RING_SIZE as u32;
        let entry_offset = 12 + (idx as usize * 8);
        ...
        // Read entry: pid (4 bytes) + task_type (1 byte) + padding (3 bytes)
        let pid = u32::from_ne_bytes([
            ring_bytes[entry_offset],
            ring_bytes[entry_offset + 1],
            ring_bytes[entry_offset + 2],
        ...
        if task_type_val == TASK_TYPE_LC || task_type_val == TASK_TYPE_BE {
            task_type_map
                .update(&pid.to_ne_bytes(), &[task_type_val], MapFlags::ANY)
                .context("Failed to update task_type_by_pid map")?;
        } else {
            // Remove entry if invalid type
            task_type_map
                .delete(&pid.to_ne_bytes())
                .context("Failed to delete from task_type_by_pid map")?;
        }
    ...
    if processed > 0 {
        let mut new_ring_bytes = ring_bytes.to_vec();
        let consumer_bytes = consumer.to_ne_bytes();
        new_ring_bytes[8..12].copy_from_slice(&consumer_bytes);

        ring_map
            .update(&key, &new_ring_bytes, MapFlags::ANY)
            .context("Failed to update ring buffer consumer")?;
    }
